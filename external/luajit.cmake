...
if(NOT ${CMAKE_HOST_SYSTEM_PROCESSOR} STREQUAL "x86_64")
	...
	set(LUAJIT_CFLAGS ${LUAJIT_CFLAGS} -funwind-tables)
	set(LUAJIT_XCFLAGS ${LUAJIT_XCFLAGS} -DLUAJIT_UNWIND_EXTERNAL)
endif()

if(CMAKE_BUILD_TYPE STREQUAL Debug)
	set(LUAJIT_XCFLAGS ${LUAJIT_XCFLAGS} -DLUA_USE_APICHECK) # -DLUAJIT_USE_GDBJIT -DLUA_USE_ASSERT
	set(LUAJIT_CFLAGS ${LUAJIT_CFLAGS} -O0 -g)
	set(LUAJIT_CCDEBUG ${LUAJIT_CCDEBUG} -g)
elseif(CMAKE_BUILD_TYPE STREQUAL Memcheck)
	set(LUAJIT_XCFLAGS ${LUAJIT_XCFLAGS} -DLUAJIT_USE_VALGRIND -DLUA_USE_APICHECK) # -DLUAJIT_USE_GDBJIT
	set(LUAJIT_CFLAGS ${LUAJIT_CFLAGS} -O0 -g)
	set(LUAJIT_CCDEBUG ${LUAJIT_CCDEBUG} -g)
...
execute_process(COMMAND echo TARGET_CFLAGS="${LUAJIT_CFLAGS}" TARGET_XCFLAGS="${LUAJIT_XCFLAGS}" TARGET_LDFLAG="${LUAJIT_LDFLAGS}"
...
add_custom_target(luajit
	COMMAND make -C ${CMAKE_CURRENT_BINARY_DIR}/luajit PREFIX=${INSTALL_FULLDIR} BUILDMODE=dynamic
		TARGET_CFLAGS="${LUAJIT_CFLAGS}" TARGET_XCFLAGS="${LUAJIT_XCFLAGS}" TARGET_LDFLAG="${LUAJIT_LDFLAGS}"
		CCDEBUG="${LUAJIT_CCDEBUG}"
	COMMAND make -C ${CMAKE_CURRENT_BINARY_DIR}/luajit LDCONFIG='/sbin/ldconfig -n' PREFIX=${INSTALL_FULLDIR}
		BUILDMODE=dynamic TARGET_CFLAGS="${LUAJIT_CFLAGS}" TARGET_XCFLAGS="${LUAJIT_XCFLAGS}"
		TARGET_LDFLAG="${LUAJIT_LDFLAGS}" CCDEBUG="${LUAJIT_CCDEBUG}"
