...
function grammar.new(name, def, ccomp)
	...
	if ccomp then
		-- Create c grammar
		f = assert(io.open(c_file_name, "w"))
...
int parse(lua_State *L)
{
	...
	lua_getfield(L, 1, "my_parse");
	for (i = 0; i < top; i++) lua_pushvalue(L, i+1);

	if (lua_pcall(L, top, 2, h)) {
		lua_state_print_error(L, "parse");
		return 0;
	}
...
int luaopen_%s(lua_State *L)
{
	...
	if ccomp then
		...
		-- Compile c grammar
		local so_file_name = file_name..".so"
		local compile_command = "gcc -shared -g -Wall -o "..so_file_name.." -fPIC "..c_file_name
		log.info("compiling grammar '%s': %s", name, compile_command)
		os.execute(compile_command)
		local c_grammar = require(file_name)

		-- Glue c grammar with lua grammar
		for _, value in pairs(g._exports) do
			value.lua_parse = value.parse
			value.parse = c_grammar.parse
		end
	end
