...
struct module *module_load(const char *module_name, struct parameters *args)
{
	...
	if (module->api_version != HAKA_API_VERSION) {
		LOG_INFO(SECTION_CORE, "%s: invalid API version", full_module_name);
	...
	if (atomic_get(&module->ref) == 0) {
		...
		if (module->name && module->author) {
			LOG_INFO(SECTION_CORE, "load module '%s', %s, %s",
			...
			LOG_INFO(SECTION_CORE, "load module '%s'", full_module_name);
		}
...
void module_release(struct module *module)
{
	if (atomic_dec(&module->ref) == 0) {
		/* Cleanup the module */
		LOG_INFO(SECTION_CORE, "unload module '%s'", module->name);
		module->cleanup();
		dlclose(module->handle);
	}
}
