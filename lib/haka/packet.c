...
int packet_receive(struct packet **pkt)
{
	...
	if (!ret && *pkt) {
		...
		assert(vbuffer_isvalid(&(*pkt)->payload));
		LOG_DEBUG(SECTION_PACKET, "received packet id=%lli",
...
void packet_drop(struct packet *pkt)
{
	assert(packet_module);
	assert(pkt);
	LOG_DEBUG(SECTION_PACKET, "dropping packet id=%lli",
			packet_module->get_id(pkt));
...
void packet_accept(struct packet *pkt)
{
	...
	LOG_DEBUG(SECTION_PACKET, "accepting packet id=%lli",
			packet_module->get_id(pkt));
...
bool packet_send(struct packet *pkt)
{
	...
	LOG_DEBUG(SECTION_PACKET, "sending packet id=%lli",
		packet_module->get_id(pkt));
