...
local function ensure_used_c_grammar(fname)
	assert(io.open(haka.config.ccomp.runtime_dir..fname.."_grammar.so", "r"))
end
...
function TestGrammarCompilation:test_new_grammar_create_c_file(fname)
	...
	-- Then
	local f = assert(io.open(haka.config.ccomp.runtime_dir..fname.."_grammar.c", "r"))
