...
int apply_iptables(const char *table, const char *conf, bool noflush)
{
	...
	if (child_pid == 0) {
		...
		if (!output) {
			LOG_ERROR(capture, "iptables-restore: %s", errno_error(errno));
			return errno;
		}
		...
		while ((line_size = getline(&buffer, &buffer_size, output)) >= 0) {
			if (line_size > 1) {
				...
				LOG_INFO(capture, "iptables-restore: %s", buffer);
			}
		}
...
int save_iptables(const char *table, char **conf, bool all_targets)
{
	...
	if (child_pid == 0) {
		...
		if (!input || !err) {
			LOG_ERROR(capture, "iptables-save: %s", errno_error(errno));
			fclose(input);
			fclose(err);
			return errno;
		}
		...
		while (fd_count > 0) {
			...
			if (FD_ISSET(pipefd_err[1], &curfds)) {
				...
				if (line_size > 1) {
					buffer[line_size-1] = '\0';
					LOG_INFO(capture, "iptables-save: %s", buffer);
				}
				else {
