		...
		struct nfq_data *nfad, void *data)
{
	...
	if (!packet_hdr) {
		LOG_ERROR(capture, "unable to get packet header");
		return 0;
	}

	packet_len = nfq_get_payload(nfad, &packet_data);
	if (packet_len > PACKET_BUFFER_SIZE) {
		LOG_WARNING(capture, "received packet is too large");
		return 0;
	}
	else if (packet_len < 0) {
		LOG_ERROR(capture, "unable to get packet payload");
		return 0;
	}
...
static int open_send_socket(bool mark)
{
	...
	if (fd < 0) {
		LOG_ERROR(capture, "cannot open send socket: %s", errno_error(errno));
		return -1;
	}

	if (setsockopt(fd, IPPROTO_IP, IP_HDRINCL, &one, sizeof(one)) < 0) {
		close(fd);
		LOG_ERROR(capture, "cannot setup send socket: %s", errno_error(errno));
		return -1;
	}
...
static struct capture_module_state *init_state(int thread_id)
{
	...
	if (!state->handle) {
		LOG_ERROR(capture, "unable to open nfqueue handle");
		cleanup_state(state);
		return NULL;
	}
	...
	for (i=0; i<sizeof(proto_family)/sizeof(proto_family[0]); ++i) {
		if (nfq_unbind_pf(state->handle, proto_family[i]) < 0) {
			LOG_ERROR(capture, "cannot unbind queue");
			cleanup_state(state);
			return NULL;
		}

		if (nfq_bind_pf(state->handle, proto_family[i]) < 0) {
			LOG_ERROR(capture, "cannot bind queue");
			cleanup_state(state);
			return NULL;
		}
	...
	if (!state->queue) {
		LOG_ERROR(capture, "cannot create queue");
		cleanup_state(state);
		return NULL;
	}
	...
	if (nfq_set_mode(state->queue, NFQNL_COPY_PACKET,
		...
		LOG_ERROR(capture, "cannot set mode to copy packet");
		cleanup_state(state);
		return NULL;
	}
	...
	if (nfq_set_queue_maxlen(state->queue, nfqueue_len) < 0) {
		LOG_WARNING(capture, "cannot change netfilter queue len");
	}
...
static int open_pcap(struct pcap_dump *pcap, const char *file)
{
	if (file) {
		...
		if (!pcap->pd) {
			LOG_ERROR(capture, "cannot setup pcap sink");
			return 1;
		}
...
static void restore_iptables()
{
	if (iptables_saved) {
		if (apply_iptables("raw", iptables_saved, !iptables_save_need_flush) != 0) {
			LOG_ERROR(capture, "cannot restore iptables rules");
		}
	}
}
...
static int init(struct parameters *args)
{
	...
	if (save_iptables("raw", &iptables_saved, install)) {
		LOG_ERROR(capture, "cannot save iptables rules");
		cleanup();
		return 1;
	}
...

	{
		...
		if (!interfaces || strlen(interfaces) == 0) {
			LOG_ERROR(capture, "no interfaces selected");
			cleanup();
			return 1;
		}
	...
	if (!ifaces) {
		LOG_ERROR(capture, "memory error");
		free(interfaces_buf);
		cleanup();
		return 1;
	}

	LOG_INFO(capture, "installing iptables rules for device(s) %s", interfaces_buf);

	{
		...
		if (getifaddrs(&ifa)) {
			LOG_ERROR(capture, "%s", errno_error(errno));
			free(interfaces_buf);
			cleanup();
			return 1;
		}
		...
		for (index = 0, str = interfaces_buf; index < count; index++, str = NULL) {
			...
			if (!is_iface_valid(ifa, token)) {
				LOG_ERROR(capture, "'%s' is not a valid network interface", token);
				free(interfaces_buf);
				cleanup();
				return 1;
			}
	...
	if (!install) {
		LOG_WARNING(capture, "iptables setup rely on user rules");
	}

	new_iptables_config = iptables_config(ifaces, thread_count, install);
	if (!new_iptables_config) {
		LOG_ERROR(capture, "cannot generate iptables rules");
		free(ifaces);
		free(interfaces_buf);
		cleanup();
		return 1;
	...
	if (apply_iptables("raw", new_iptables_config, !install)) {
		LOG_ERROR(capture, "cannot setup iptables rules");
	...
	if (dump) {
		...
		if (!(file_in || file_out)) {
			LOG_WARNING(capture, "no dump pcap files specified");
		}
		else {
			...
			if (file_in) {
				...
				LOG_INFO(capture, "dumping received packets into '%s'", file_in);
			}
			if (file_out) {
				open_pcap(&pcap->out, file_out);
				LOG_INFO(capture, "dumping emitted packets into '%s'", file_out);
			}
		}
	}
...
static int packet_do_receive(struct capture_module_state *state, struct packet **pkt)
{
	...
	if (rv <= 0) {
		if (rv == -1 && errno != EINTR) {
			LOG_ERROR(capture, "packet reception failed, %s", errno_error(errno));
		}
		return 0;
	}
	...
	if (FD_ISSET(state->fd, &read_set)) {
		...
		if (nfq_handle_packet(state->handle, state->receive_buffer, rv) == 0) {
			...
			LOG_ERROR(capture, "packet processing failed");
			return 0;
		}
	}
...
static void packet_verdict(struct packet *orig_pkt, filter_result result)
{
	...
	if (vbuffer_isvalid(&pkt->core_packet.payload)) {
		...
		if (pkt->id == -1) {
			...
			default:
				LOG_DEBUG(capture, "unknown verdict");
				verdict = NF_DROP;
				break;
			}
		...
		if (ret == -1) {
			LOG_ERROR(capture, "packet verdict failed");
		}
