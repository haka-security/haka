...
static int init(struct parameters *args)
{
	...
	if (interfaces) {
		...
		LOG_ERROR(capture, "specifiy either a device or a pcap filename");
		cleanup();
		return 1;
	}
...
static bool open_pcap(struct pcap_capture *pd, const char *input, bool isiface)
{
	...
	if (isiface) {
		LOG_INFO(capture, "listening on device %s", input);

		pd->pd = pcap_open_live(input, SNAPLEN, 1, 0, errbuf);
		if (pd->pd && (strlen(errbuf) > 0)) {
			LOG_WARNING(capture, "%s", errbuf);
		}
	}
	else {
		LOG_INFO(capture, "opening file '%s'", input);
	...
	{
		LOG_ERROR(capture, "%s", pcap_geterr(pd->pd));
		return false;
	}
	...
	case DLT_PPP:
	default:
		LOG_ERROR(capture, "%s", "unsupported data link");
		return false;
	}
...
static pcap_dumper_t *open_dump_file(struct capture_module_state *state, const char *filename)
{
	...
	if (!dump) {
		cleanup_state(state);
		LOG_ERROR(capture, "unable to dump on %s", filename);
		return NULL;
	}
		...
		*pkt = (struct packet *)packet;
		...
		if (ret < 0) {
			...
			if (ret == -1) {
				...
				LOG_ERROR(capture, "skipping malformed packet %llu", ++state->packet_id);
				return 0;
			}
			else {
				...
				if (packet->header.caplen < packet->header.len)
					LOG_WARNING(capture, "packet truncated");
				...
				if (pd->file) {
					...
					if (time_isvalid(&pd->last_progress)) {
						...
						if (difftime.secs >= PROGRESS_DELAY) /* 5 seconds */
						{
							...
							if (percent > 0) {
								LOG_INFO(capture, "progress %.2f %%", percent);
							}
						}
					}
					else {
				...
				if (!packet_build_payload(packet)) {
					LOG_ERROR(capture, "malformed packet %llu", packet->id);
...
static const char *packet_get_dissector(struct packet *orig_pkt)
{
	...
	case -1:
		LOG_ERROR(capture, "malformed packet %llu", pkt->id);
