...
static REGISTER_LOG_SECTION(pcap);
...
static int init(struct parameters *args)
{
	...
	if (interfaces) {
		...
		LOG_ERROR(pcap_section, "specifiy either a device or a pcap filename");
		cleanup();
		return 1;
	}
...
static bool open_pcap(struct pcap_capture *pd, const char *input, bool isiface)
{
	...
	if (isiface) {
		LOG_INFO(pcap_section, "listening on device %s", input);

		pd->pd = pcap_open_live(input, SNAPLEN, 1, 0, errbuf);
		if (pd->pd && (strlen(errbuf) > 0)) {
			LOG_WARNING(pcap_section, "%s", errbuf);
		}
	}
	else {
		LOG_INFO(pcap_section, "opening file '%s'", input);
	...
	if (!pd->pd) {
		LOG_ERROR(pcap_section, "%s", errbuf);
		return false;
	}
	...
	{
		LOG_ERROR(pcap_section, "%s", pcap_geterr(pd->pd));
		return false;
	}
	...
	case DLT_PPP:
	default:
		LOG_ERROR(pcap_section, "%s", "unsupported data link");
		return false;
	}
...
static pcap_dumper_t *open_dump_file(struct packet_module_state *state, const char *filename)
{
	...
	if (!dump) {
		cleanup_state(state);
		LOG_ERROR(pcap_section, "unable to dump on %s", filename);
		return NULL;
	}
	...
	if (state->sent_head) {
		...
		*pkt = (struct packet *)packet;
		...
		for (i=0; i<state->pd_count; ++i) {
			...
			if (fd < 0) {
				LOG_ERROR(pcap_section, "%s", errno_error(errno));
				return 1;
			}
			else {
				FD_SET(fd, &read_set);
		...
		if (ret < 0) {
			...
			if (ret == -1) {
				...
				LOG_ERROR(pcap_section, "skipping malformed packet %llu", ++state->packet_id);
				return 0;
			}
			else {
				...
				if (packet->header.caplen < packet->header.len)
					LOG_WARNING(pcap_section, "packet truncated");
				...
				if (pd->file) {
					...
					if (time_isvalid(&pd->last_progress)) {
						...
						if (difftime.secs >= PROGRESS_DELAY) /* 5 seconds */
						{
							...
							if (percent > 0) {
								LOG_INFO(pcap_section, "progress %.2f %%", percent);
							}
						}
					}
					else {
				...
				if (!packet_build_payload(packet)) {
					LOG_ERROR(pcap_section, "malformed packet %llu", packet->id);
...
static const char *packet_get_dissector(struct packet *orig_pkt)
{
	...
	case -1:
		LOG_ERROR(pcap_section, "malformed packet %llu", pkt->id);
