...
void prepare(int threadcount, bool attach_debugger, bool dissector_graph)
{
	...
	if (packet_module->pass_through()) {
		LOG_INFO(SECTION_CORE, "setting packet mode to pass-through\n");
		packet_set_mode(MODE_PASSTHROUGH);
	}

	LOG_INFO(SECTION_CORE, "loading rule file '%s'", configuration_file);
	...
	if (threadcount > 1) {
		LOG_INFO(SECTION_CORE, "starting multi-threaded processing on %i threads\n", threadcount);
	}
	else {
		LOG_INFO(SECTION_CORE, "starting single threaded processing\n");
	}
}
